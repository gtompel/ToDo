...
export async function POST(req: Request) {
  ...
  if (!(user.role === "ADMIN" || user.role === "TECHNICIAN")) return NextResponse.json({ error: "Недостаточно прав" }, { status: 403 })
  ...
  if (contentType.includes('multipart/form-data')) {
    try {
      ...
      keepEntries.forEach((k) => { if (typeof k === 'string') keepAttachments.push(k) })
      ...
      return NextResponse.json({ error: 'Ошибка при обновлении запроса', details: String(e) }, { status: 500 })
    }
  } else {
