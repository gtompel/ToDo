...
export default function NotificationsCenterPage() {
  ...
  const filteredStats = notifications.filter(n =>
    period === 'all' ? true : isAfter(new Date(n.createdAt), periodStart)
  )
    ...
    fetch("/api/settings")
      ...
      .then(data => {
        setSettings({
          ...
          quietHours: data.settings?.quietHours ?? false,
          signature: data.settings?.signature ?? 'С уважением, ITSM',
        })
      })
  ...
  // === ФИЛЬТРАЦИЯ ПОЛЬЗОВАТЕЛЕЙ В ДИАЛОГЕ ===
  const filteredUsers = useMemo(() => {
    const q = userSearch.trim().toLowerCase()
    if (!q) return usersList
    return usersList.filter(u => {
      const fio = `${u.lastName || ''} ${u.firstName || ''} ${u.middleName || ''}`.toLowerCase()
      return fio.includes(q) || (u.email && u.email.toLowerCase().includes(q))
    })
  ...
  const filtered = useMemo(() => {
    return notifications.filter(n => {
      ...
      if (search) {
        ...
        if (
          !(n.title?.toLowerCase().includes(lowerSearch) || n.message?.toLowerCase().includes(lowerSearch))
        ) return false
      }
      if (isAdmin && userFilter && n.user) {
        ...
        if (!(fio.includes(lowerFilter) || n.user.email?.toLowerCase().includes(lowerFilter))) return false
      }
      return true
    })
  ...
  return (
    <div className="max-w-5xl mx-auto py-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Центр уведомлений {isAdmin && <span className="text-xs text-blue-600 ml-2">(Администратор)</span>}
          </CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <div>
              <div className="flex flex-col md:flex-row md:items-center gap-2 mb-4">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant={filter === "all" ? "default" : "outline"} onClick={() => { setFilter("all"); setPage(1) }}>
                    Все
                  </Button>
                  <Button size="sm" variant={filter === "unread" ? "default" : "outline"} onClick={() => { setFilter("unread"); setPage(1) }}>
                    Непрочитанные
                  </Button>
                  <Button size="sm" variant={filter === "read" ? "default" : "outline"} onClick={() => { setFilter("read"); setPage(1) }}>
                    Прочитанные
                  </Button>
                </div>
              ...
              {loading ? (
                ...
                <div className="space-y-2">
                  ...
                  {paged.map(n => (
                    <div
                      key={n.id}
                      className={`flex items-start gap-3 p-3 border rounded bg-muted/50 ${
                        isAdmin && selected.includes(n.id) ? 'ring-2 ring-blue-400' : ''
                      }`}
                    >
                      {isAdmin && (
                        <input
                      ...
                      <button
                        ...
                        className="mt-1"
                        title={n.read ? "Прочитано" : "Отметить как прочитанное"}
                      >
                        {n.read ? (
                      ...
                      <div className="flex-1">
                        ...
                        {isAdmin && n.user && (
                          <div className="text-xs text-muted-foreground mt-1">
                            {n.user.lastName} {n.user.firstName} {n.user.middleName} ({n.user.email})
                          </div>
                        )}
                      </div>
                      <div className="text-xs text-muted-foreground whitespace-nowrap ml-2 mt-1">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      <Switch
                        ...
                        onCheckedChange={v => saveSettings({ emailNotifications: v })}
                    ...
                    <div className="flex items-center gap-4">
                      <Switch
                        ...
                        onCheckedChange={v => saveSettings({ pushNotifications: v })}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-4">
                        <Switch
                          ...
                          onCheckedChange={v => saveSettings({ quietHours: v })}
                          id="quiet-hours"
                          disabled={settingsSaving}
                        />
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={e => saveSettings({ signature: e.target.value })}
          ...
          {tab === "templates" && (
            <div>
              Вкладка &quot;Шаблоны&quot; (email-уведомления) — будет реализовано далее
            </div>
          )}
          {tab === "escalation" && (
            <div>
              Вкладка &quot;Эскалация&quot; (правила эскалации) — будет реализовано далее
            </div>
          )}
        </CardContent>
      </Card>
                  ...
                  {bulkMode
                    ? bulkUsers
                        .map(id => {
                          ...
                          return u ? `${u.lastName} ${u.firstName} (${u.email})` : ''
                        })
                        .filter(Boolean)
                        .join(", ")
                    : (() => {
                        const u = usersList.find(u => u.id === createUser)
                        return u ? `${u.lastName} ${u.firstName} (${u.email})` : ''
                      })()}
                </div>
                <div className="font-bold text-lg mb-2">{createTitle}</div>
                <div className="whitespace-pre-line">{createMessage}</div>
                <Button variant="outline" onClick={() => setPreviewMode(false)} className="mt-4">
                ...
                {bulkMode ? (
                  <div>
                    ...
                    <Input
                      ...
                      className="mb-2"
                      disabled={createLoading || usersLoading}
                    />
          ...
          <DialogFooter>
            {previewMode ? (
              <Button
                ...
                disabled={
                  createLoading ||
                  (bulkMode ? bulkUsers.length === 0 : !createUser) ||
                  !createTitle ||
                  !createMessage
                }
              >
                {createLoading ? "Создание..." : bulkMode ? "Разослать" : "Создать"}
              </Button>
            ) : (
              <div className="flex gap-2">
                <Button
                ...
                >
                  {bulkMode ? "Разослать" : "Создать"}
                </Button>
                <Button
                  variant="outline"
                  onClick={() => setPreviewMode(true)}
                  disabled={
                    (bulkMode ? bulkUsers.length === 0 : !createUser) || !createTitle || !createMessage
                  }
                >
                  Предпросмотр
                </Button>
