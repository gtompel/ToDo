...
export const AiAssistantChat: React.FC<AiAssistantChatProps> = ({ open, onClose }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full max-w-md z-[100] animate-fade-in">
      <div className="mx-auto my-8 bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden relative animate-fade-in">
        <div className="flex items-center gap-2 px-6 py-4 border-b bg-blue-50">
          <BotIcon className="text-blue-600" />
          <div className="text-base font-bold">AI ассистент</div>
          <span className="ml-auto flex items-center gap-1 text-xs">
            {status === 'checking' && <Loader2 className="animate-spin w-4 h-4 text-gray-400" />}
          ...
          <button className="ml-2 p-1 rounded hover:bg-blue-100" title="Свернуть" onClick={() => setMinimized(true)}><Minus size={18} /></button>
          <button className="ml-1 p-1 rounded hover:bg-red-100" title="Закрыть" onClick={onClose}><X size={18} /></button>
        </div>
        <div className="flex flex-col h-96 px-6 py-4 overflow-y-auto bg-white transition-all duration-200">
          {messages.length === 0 && (
            <div className="text-gray-400 text-center my-auto select-none">Введите вопрос для AI...</div>
          )}
          {messages.map((msg, i) => (
            <div key={i} className={`mb-2 flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`rounded-2xl px-4 py-2 max-w-[80%] text-sm shadow ${msg.role === 'user' ? 'bg-blue-100 text-blue-900' : 'bg-gray-100 text-gray-800'}`}>{msg.text}</div>
            </div>
          ))}
          {loading && (
            <div className="flex justify-start mb-2">
              <div className="rounded-2xl px-4 py-2 max-w-[80%] text-sm bg-gray-100 text-gray-800 flex items-center gap-2">
                ...
                <span className="ml-2 text-xs text-gray-400">AI печатает...</span>
              </div>
            </div>
          )}
          <div ref={bottomRef} />
        ...
        <form
          className="flex gap-2 border-t px-6 py-4 bg-white"
